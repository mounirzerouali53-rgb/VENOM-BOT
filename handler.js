...
const handleMessage = async (sock, msg) => {
  try {
              ...
              if (!senderIsAdmin && !senderIsOwner) {
                ...
                if (action === 'delete') {
                  ...
                  if (botIsAdmin) {
                    ...
                    await sock.sendMessage(from, {
                      text: `🚫 *تم رصد منشن جماعي!*\n\n${usernames.join(', ')} تم طرده بسبب عمل منشن جماعي.`,
                      mentions: [sender],
                    }, { quoted: msg });
                  }
                }
    ...
    if (command.modOnly && !isMod(sender) && !isOwner(sender)) {
      return sock.sendMessage(from, { text: '*🔒 هذا الأمر مخصص للمطور فقط*!' }, { quoted: msg });
    }
    ...
    for (const participant of participants) {
      ...
      if (action === 'add' && groupSettings.welcome) {
        try {
          ...
          let message = groupSettings.welcomeMessage || 'اهلا بك @user في @group! 👋\nنتمنى ان تستمتع معنا!';
        ...
        try {
          ...
          const goodbyeMsg = `*وداعا @${displayName} 👋 لن نفتقدك أبدا*!`;
          ...
          const goodbyeMsg = `وداعا @${participantNumber} وداعا لن نفتقدك أبدا! 💀*`;
      ...
      for (const call of calls) {
        if (call.status === 'offer') {
          ...
          await sock.sendMessage(call.from, {
            text: '*🚫 المكالمات محظورة في الخاص سيتم حظرك*'
          });
        }
      }
    } catch (err) {
